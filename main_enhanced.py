...
def draw_enhanced_overlay(frame, mode_controller, tracking_status):
    ...
    cv2.putText(
        status_bar,
        "C:Record | S:Stop | D:Describe | M:Mode | Q:Quit",
        (width - 620, 30),
        config.FONT,
        0.5,
        (200, 200, 200),
    ...
    for obj in mode_controller.object_manager.objects:
        if obj.bbox:
            ...
            cv2.putText(frame, label_text, (x, y - 10), config.FONT, 0.6, obj.color, 2)
            ...
            if obj.predicted_bbox and config.MOTION_PREDICTION_ENABLED:
                ...
                cv2.rectangle(
                    frame, (px, py), (px + pw, py + ph), obj.color, 1, cv2.LINE_4
                )
                cv2.line(
                    frame,
                    (x + w // 2, y + h // 2),
    ...
    for obj in mode_controller.object_manager.objects:
        if obj.bbox:
            ...
            indicator_radius = 6
            cv2.circle(
                frame, (indicator_x, indicator_y), indicator_radius, obj.color, -1
            )
            cv2.circle(
                frame,
    ...
    def __exit__(self, exc_type, value, traceback):
        if self.enabled:
            self.termios.tcsetattr(
                self.sys.stdin, self.termios.TCSADRAIN, self.old_settings
            )
...
def main():
    ...
    ui_enabled = (
        bool(os.environ.get("DISPLAY")) and os.environ.get("NOVA_HEADLESS") != "1"
    )
    ...
    try:
        ...
        with console:
            ...
            if not ui_enabled:
                print(
                    "   GUI display unavailable or headless mode requested. Running in terminal-only mode."
                )

            print("\n[4/7] Initializing Mode Controller...")
            mode_controller = ModeController()
            mode_controller.set_frame_dimensions(
                vision_controller.frame_width, vision_controller.frame_height
            )
            ...
            if config.ENABLE_LEARNING:
                ...
                print(
                    f"   [LEARNING] Learned: {stats['total_detections']} detections, {stats['unique_labels']} object types"
                )
                print(
                    f"   [LEARNING] Cache: {stats['cached_images']} images ({stats['cache_size_mb']:.1f} MB)"
                )
            ...
            if (
                voice_enabled
                and voice_controller
                and not getattr(voice_controller, "groq_client", None)
            ):
                print(
                    "   Voice commands unavailable because Groq STT could not initialize."
                )
            ...
            if (
                voice_enabled
                and config.ENABLE_CHAT_PERSONA
                and not getattr(voice_controller, "chat_persona_enabled", False)
            ):
            ...
            if (
                voice_enabled
                and voice_controller
                and getattr(voice_controller, "chat_persona_enabled", False)
            ):
            ...
            def vision_worker():
                ...
                while shared_state.is_running:
                    ...
                    if detections is not None:
                        print(
                            f"[VISION] Async detection complete. Processing {len(detections)} objects..."
                        )
                        ...
                        if count > 0:
                            if mode_controller.should_filter_objects():
                                ...
                                mode_controller.object_manager.filter_by_labels(
                                    filter_labels
                                )
                            ...
                            if voice_enabled and voice_controller:
                                voice_controller.speak(
                                    f"Found {count} objects", async_mode=True
                                )
                    ...
                    if should_detect:
                        if not vision_controller.is_searching:
                            ...
                            vision_controller.start_reacquisition_multi(
                                small_frame, prompt
                            )
                        else:
                            print(
                                "[WARNING] Detection already in progress, skipping request."
                            )
                    ...
                    for obj in mode_controller.object_manager.objects:
                        if obj.is_lost and obj.template is not None:
                            success, new_bbox = (
                                vision_controller.attempt_local_recovery(frame, obj)
                            )
                            if success:
                                obj.bbox = new_bbox
                                mode_controller.object_manager.init_tracker(
                                    obj.id, frame
                                )
                                obj.is_lost = False
                                obj.lost_time = None
                                mode_controller.object_manager.update_template(
                                    obj, frame
                                )
                    ...
                    if mode_controller.object_manager.cleanup_stale_trackers(
                        max_age=30.0
                    ):
                    ...
                    if mode_controller.check_lost_threats():
                        ...
                        if voice_enabled and voice_controller:
                            voice_controller.speak(
                                "Lost track. Rescanning.", async_mode=True
                            )
                ...
                while True:
                    ...
                    if not ret or frame is None:
                        ...
                        if consecutive_frame_failures >= max_frame_failures:
                            print(
                                f"[ERROR] Failed to read frame {max_frame_failures} times. Exiting."
                            )
                            break
                        time.sleep(0.01)
                        continue
                    ...
                    frame = cv2.resize(
                        rotated, (config.CAMERA_WIDTH, config.CAMERA_HEIGHT)
                    )
                    ...
                    if key == ord("f") or key == ord("F"):
                        ...
                        if (
                            voice_enabled
                            and voice_controller
                            and voice_controller.is_recording
                        ):
                            if voice_command_busy:
                                ...
                                def process_voice_thread(qa_frame, live_frame):
                                    ...
                                    try:
                                        ...
                                        if (
                                            audio_controller
                                            and voice_recording_audio_paused
                                        ):
                                            audio_controller.resume_stream()
                                            voice_recording_audio_paused = False
                                        ...
                                        if intent == "track_object":
                                            ...
                                            voice_controller.speak(
                                                f"Tracking {obj_name}", async_mode=True
                                            )
                                            shared_state.add_command("detect")
                                            ...
                                            if mode_controller.set_mode(mode_name):
                                                voice_controller.speak(
                                                    f"{mode_name} mode activated",
                                                    async_mode=True,
                                                )
                                                shared_state.add_command("detect")
                                        elif intent == "describe_scene":
                                            print("[VISION] Describing scene...")
                                            vision_controller.describe_scene(
                                                live_frame, voice_controller
                                            )
                                            ...
                                            if learning_module:
                                                print(
                                                    f"[LEARNING] Recalling: {obj_name}..."
                                                )
                                                recall_info = (
                                                    learning_module.recall_object(
                                                        obj_name
                                                    )
                                                )
                                                if recall_info:
                                            ...
                                            mode_controller.object_manager.clear()
                                            voice_controller.speak(
                                                "Tracking stopped", async_mode=True
                                            )
                                            shared_state.update_tracking([], "READY")
                                        elif intent == "help":
                                            voice_controller.speak(
                                                voice_controller.get_help_text(),
                                                async_mode=False,
                                            )
                                        elif intent == "quit":
                                            voice_controller.speak(
                                                "Goodbye", async_mode=True
                                            )
                                            time.sleep(1)
                        ...
                        try:
                            current_idx = available_modes.index(
                                mode_controller.current_mode
                            )
                        except ValueError:
                            current_idx = -1

                        next_mode = available_modes[
                            (current_idx + 1) % len(available_modes)
                        ]
